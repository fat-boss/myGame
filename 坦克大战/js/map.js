...
var Map=function(wCtx,gCtx){
	this.level=1;
	this.mapLevel=null;
	this.wallCtx=wCtx;//墙画布
	this.grassCtx=gCtx;//草坪画布
	...
	this.num=new Num(this.wallCtx);//画数字
	this.mapWidth=416;
	this.mapHeight=416;

	this.setMapLevel=function(level){
		this.level=level;
		var tempMap=eval("map"+this.level);//临时地图
		this.mapLevel=new Array();//地图等级
	...
	this.clearEnemyNum=function(totalEnemyNum,enemyNum){
		...
		var tempY = y + (Math.ceil(totalEnemyNum/2)-1)*enemySize - (parseInt((enemyNum-1)/2))*enemySize;
		this.wallCtx.fillRect(tempX,tempY,14,14);
	}

	//画坦克的生命数
	//lives生命数 type坦克类型 1,代表玩家1 2，代表玩家2
	this.drawLives=function(lives,type){
		var x=482;
		var y=272;
		if(type==2){
			y=320;
	...
	//更新地图
	//indexArr 需要更新的地图索引数组，二维数组 如[[1,1],[2,2]]
	//target 更新之后的数值
